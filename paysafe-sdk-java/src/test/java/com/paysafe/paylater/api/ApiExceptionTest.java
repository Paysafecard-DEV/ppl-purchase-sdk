...
import com.paysafe.paylater.communication.standard.PaysafeBaseCommunicationTest;
 ...
 * These tests are mostly intended as examples. The responses are based on actual API calls to the Paysafe Pay Later test environment.
 */
public class ApiExceptionTest extends PaysafeBaseCommunicationTest {
    ...
    private void verifyException(Class<? extends ApiException> exceptionClass, int httpStatusCode, String paysafeStatusCode, ApiException exception) {
        verifyException(exceptionClass, httpStatusCode, paysafeStatusCode, exception, OperationStatus.ERROR);
    }
    ...
    /**
     * @return A collection of headers that the Paysafe Pay Later API can return when an error occurs.
     */
    private Map<String, String> getResponseHeaders() {
        Map<String, String> responseHeaders = new LinkedHashMap<>();
        responseHeaders.put("X-Application-Context", "proxy:test:8443");
        responseHeaders.put("Expires", "0");
        responseHeaders.put("Access-Control-Allow-Headers", "x-requested-with, authorization, cache-control, X-HTTP-Method-Override, Content-Type, Accept, paysafe-pl-tenant, paysafe-pl-public-key, paysafe-pl-secret-key");
