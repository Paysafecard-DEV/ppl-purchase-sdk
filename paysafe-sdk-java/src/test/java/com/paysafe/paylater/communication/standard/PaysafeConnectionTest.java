...
import com.paysafe.paylater.util.ReflectionUtil;

public class PaysafeConnectionTest {

    private static final int CONNECT_TIMEOUT = 10000;
    private static final int READ_TIMEOUT = 20000;
    private static final int MAX_CONNECTIONS = 100;

    public static void assertConnection(PaysafeConnection connection, int connectTimeout, int readTimeout, int maxConnections) {
        assertRequestConfig(connection, connectTimeout, readTimeout);
        assertMaxConnections(connection, maxConnections);
    }

    private static void assertRequestConfig(PaysafeConnection connection, int connectTimeout, int readTimeout) {
    ...
    private static void assertMaxConnections(PaysafeConnection connection, int maxConnections) {
    ...
    @SuppressWarnings("resource")
    private static void assertHttpsProtocols(PaysafeConnection connection, Set<String> httpsProtocols) {
        CloseableHttpClient httpClient = ReflectionUtil.getField(connection, "httpClient", CloseableHttpClient.class);
        PoolingHttpClientConnectionManager connectionManager = ReflectionUtil.getField(httpClient, "connManager", PoolingHttpClientConnectionManager.class);
    ...
    public void testConstruct() {
        ...
        PaysafeConnection connection = new PaysafeConnection(CONNECT_TIMEOUT, READ_TIMEOUT);
    ...
    public void testConstructWithMaxConnections() {
        PaysafeConnection connection = new PaysafeConnection(CONNECT_TIMEOUT, READ_TIMEOUT, MAX_CONNECTIONS);
        assertRequestConfig(connection, CONNECT_TIMEOUT, READ_TIMEOUT);
    ...
    public void testConstructWithHttpsProtocols() {
        ...
        PaysafeConnection connection = new PaysafeConnection(CONNECT_TIMEOUT, READ_TIMEOUT, MAX_CONNECTIONS, httpsProtocols);
        assertRequestConfig(connection, CONNECT_TIMEOUT, READ_TIMEOUT);
        assertMaxConnections(connection, MAX_CONNECTIONS);
